...
# current tournament
cur_tourney_data = pd.DataFrame(columns=['Seed','School'])

URL = 'https://www.sports-reference.com/cbb/postseason/{}-ncaa.html'.format(cur_year)
# URL = 'https://www.sports-reference.com/cbb/postseason/2000-ncaa.html'
page = requests.get(URL)
...
# Add year suffix to school name for merging later
merged['School'] = merged['School'].astype(str) + '_'  + cur_year

# Remove now unecessary season column
merged = merged[['Seed','School','Wins']]

# append to the main dataframe
cur_tourney_data = cur_tourney_data.append(merged)

cur_tourney_data.to_csv(root + 'data/curr_tour.csv')
